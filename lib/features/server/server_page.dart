...
void addServerLog(WidgetRef ref, String msg) {
  ...
  ref.read(serverLogProvider.notifier).state = [
    '[$ts] $msg',
    ...ref.read(serverLogProvider),
  ].take(200).toList();
}
  ...
  Future<void> toggleService(WidgetRef ref) async {
    if (state.isRunning) {
      ...
      try {
        ...
        addServerLog(ref,
            ok ? 'HTTP 服务已启动 → 0.0.0.0:${cfg.port}' : 'HTTP 服务启动失败');
      } catch (e, st) {
...
class ServerPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          _infoCard(theme, l, cfg, did),
          const SizedBox(height: 16),
          _svcCard(context, ref, theme, l, cfg, s, did),
          const SizedBox(height: 16),
          _bindCard(context, ref, theme, cfg, s, did, l),
          const SizedBox(height: 16),
          _logCard(context, theme, logs, ref),
          const SizedBox(height: 16),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.phone_android, color: t.colorScheme.primary),
              const SizedBox(width: 8),
              Text(l.deviceInfo, style: t.textTheme.titleMedium),
            ]),
            const Divider(),
            _row(l.deviceNameLabel, c.deviceName),
            _row(
              l.deviceId,
              d.when(
                data: (v) => v.substring(0, 8),
                loading: () => '...',
                error: (_, __) => 'error',
              ),
            ),
            _row(l.os, _os),
            _row(l.hostname, _host),
          ],
        ),
      ),
    );
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.cloud, color: t.colorScheme.primary),
              const SizedBox(width: 8),
              Text(l.serviceStatus, style: t.textTheme.titleMedium),
            ]),
            const Divider(),
            _statusRow(l.httpService, s.isRunning,
                s.isRunning ? l.running : l.stopped),
            _row(l.serverPort, '${c.port}'),
            _row(l.listenAddress, c.address),
            _row(l.pushInterval, l.seconds(c.pushInterval)),
            const Divider(),
            d.when(
              data: (did) => tok.when(
                data: (tk) => Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _row(l.accessUrl, 'http://{ip}:${c.port}/$did/$tk'),
                    ...
                    Row(children: [
                      Expanded(
                        child: _row(
                          l.accessTokenLabel,
                          tk.length > 16
                              ? '${tk.substring(0, 8)}...${tk.substring(tk.length - 8)}'
                              : tk,
                        ),
                      ),
                      IconButton(
                        ...
                        tooltip: l.copyToken,
                        onPressed: () {
                          Clipboard.setData(ClipboardData(text: tk));
                          ScaffoldMessenger.of(ctx).showSnackBar(
                            SnackBar(
                              content: Text(l.tokenCopied),
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                onPressed: () =>
                    ref.read(serverStatusProvider.notifier).toggleService(ref),
                icon: Icon(s.isRunning ? Icons.stop : Icons.play_arrow),
                label: Text(s.isRunning ? l.stopService : l.startService),
              ),
            ),
          ],
        ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.qr_code, color: t.colorScheme.primary),
              const SizedBox(width: 8),
              Text(l.bindDevice, style: t.textTheme.titleMedium),
            ]),
            const Divider(),
            Text(l.bindDeviceDesc,
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                onPressed: !s.isRunning
                    ...
                    : () {
                        d.whenData(
                            (did) => tok.whenData((tk) => _showQr(ctx, c, did, tk, l)));
                      },
                icon: const Icon(Icons.qr_code),
                label:
                    Text(s.isRunning ? l.showBindQr : l.startServiceFirst),
              ),
            ),
          ],
        ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.article, color: t.colorScheme.primary),
              const SizedBox(width: 8),
              Text(l.serverLog,
  ...
  // ── 网络信息卡片 ──
  Widget _netCard(BuildContext ctx, ThemeData t, AppLocalizations l) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16),
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            Row(children: [
              Icon(Icons.wifi, color: t.colorScheme.primary),
              const SizedBox(width: 8),
              Text(l.networkInfo, style: t.textTheme.titleMedium),
            ]),
            const Divider(),
            FutureBuilder<List<String>>(
              ...
              builder: (_, snap) => snap.hasData
                  ? Column(
                      children: snap.data!
                          .map((ip) => _row(l.ipAddress, ip))
                          .toList(),
                    )
                  : _row(l.ipAddress, l.detecting),
            ),
          ],
        ),
      ),
  ...
  Widget _statusRow(String label, bool ok, String status) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
