...
- `DiscoveryIntegration` 是 Discovery 和 DeviceManager 的桥梁。发现设备时从 UDP 消息中提取 `access_token`，传给 `ManagedDevice`
...
| `serverStatusProvider` | `server_page.dart` | 服务运行状态（TODO: 对接 ServerService 启停） |
...
发现消息 JSON（snake_case）：`type`, `device_id`, `device_name`, `ip`, `port`, `os`, `access_token`, `timestamp`
...
- `build.yml` — Push/PR → main 全平台构建。PR 跳过版本计算（用回退值 `dev/0`）
- `release.yml` — tag push 或 workflow_dispatch 构建 + 发布。版本名从 pubspec.yaml 自动读取
